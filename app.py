...
def upload():
    ...
    if request.method == "POST":
        ...
        # Insert new dress into dresses table
        db.execute("INSERT INTO dresses (size, color, owner) VALUES (?,?,?)",
                   size, color, session["user_id"])

        # Save image

        dress_id = (db.execute("SELECT id FROM dresses ORDER BY time_posted DESC LIMIT 1")).fetchall()[0]["id"]
        ...
        # Insert image url into dresses table
        db.execute("UPDATE dresses SET image_url = ? WHERE time_posted = (SELECT time_posted FROM dresses ORDER BY time_posted DESC LIMIT 1)", image_url)
